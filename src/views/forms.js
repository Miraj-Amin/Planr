// forms.js — all create/edit form definitions.
// Each function opens a modal, saves to db, calls onDone(record).
...
// ── PROJECT ──────────────────────────────────────────────────────────────────
export function newProjectForm(db, onDone) {
  openModal({
    title: 'New project',
    fields: [
    ...
    onSubmit: data => {
      const proj = db.insert('projects', { name: data.name, client_org: data.client_org||null, status:'active' });
      // seed an active sprint
      db.insert('sprints', {
...
export function newTaskForm(db, projectId, opts={}, onDone) {
  ...
  openModal({
    ...
    fields: [
      ...
      { key:'parent_id',     label:'Phase',        type:'select', value: opts.parent_id||'',
      ...
      { key:'deliverable_id',label:'Deliverable',  type:'select', value: opts.deliverable_id||'',
        placeholder:'Not linked to a deliverable',
      ...
      { key:'effort_min',    label:'Effort estimate', type:'select', value: opts.effort_min||'',
        options: EFFORT_OPTS },
  ...
  openModal({
    ...
    onSubmit: data => {
      const phase = db.insert('tasks', {
        ...
        status:'todo', progress:0, sort_order:999,
      });
      onDone(phase);
    },
  });
  ...
  openModal({
    ...
    onSubmit: data => {
      const meeting = db.insert('meetings', {
        ...
        date: data.date, start_time: data.start_time||'09:00',
        duration_min: data.duration_min||55, notes: data.notes||null,
...
// ── MEETING ITEM (agenda or follow-up) ───────────────────────────────────────
export function newMeetingItemForm(db, projectId, meetingId, kind, onDone) {
  ...
  openModal({
    ...
    onSubmit: data => {
      // create the task
      const task = db.insert('tasks', {
        project_id: projectId, type: kind,
        ...
        effort_min: 0, priority: 'normal', progress: 0, flagged: true, sort_order: 999,
      });
      // create the meeting_item linking task to meeting
      const mi = db.insert('meeting_items', {
        task_id: task.id, kind, resolved: false, carried_from: null,
      });
...
// ── QUICK TASK (inline, minimal fields) ──────────────────────────────────────
  ...
  openModal({
    ...
    onSubmit: data => {
      const task = db.insert('tasks', {
        ...
        effort_min:0, priority:'normal', progress:0, flagged:false, sort_order:999,
        ...defaults,
      });
      onDone(task);
    },
