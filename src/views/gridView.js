// gridView.js — inline-editable spreadsheet grid.
// Groups tasks by deliverable. Click any cell to edit inline.
// Enter = new row below. Tab on name = indent. Shift+Tab = outdent.
...
// Module-level grid state (persists across rerenders)
let G = {
  cols:   ALL_COLS.map(c => c.id),
  widths: Object.fromEntries(ALL_COLS.map(c => [c.id, c.w])),
  pendingFocusId: null,  // task id to focus after next render
};
...
function makeEditor(colId, task, ctx, onSave, onEnter, onTabDir) {
  ...
  if (['name','duration_days','progress'].includes(colId)) {
  ...
  if (['start_date','end_date'].includes(colId)) {
    const el = document.createElement('input');
    el.type='date'; el.value=task[colId]||''; el.style.cssText=b;
  ...
  if (colId==='status') {
    ...
    const bk=document.createElement('option');bk.value='';bk.textContent='Unassigned';if(!task.owner_id)bk.selected=true;el.appendChild(bk);
    ...
    const bk=document.createElement('option');bk.value='';bk.textContent='Backlog';if(!task.sprint_id)bk.selected=true;el.appendChild(bk);
...
function rowHTML(task, indent, ctx) {
  const bg    = task.type==='phase'?'rgba(83,74,183,.04)':task.type==='deliverable'?'rgba(29,158,117,.03)':task.type==='milestone'?'rgba(186,117,23,.03)':'#fff';
  const strip = task.type==='phase'?'#534AB7':task.type==='deliverable'?'#1D9E75':task.type==='milestone'?'#BA7517':'transparent';
  const cells = G.cols.map(colId => {
    ...
    const dragHandle = colId==='name'?`<span class="row-drag" style="opacity:0;cursor:grab;color:#D1D5DB;margin-right:5px;font-size:12px;flex-shrink:0;user-select:none">⠿</span>`:'';
    return `<td class="g-cell" data-id="${task.id}" data-col="${colId}"
      style="position:relative;height:36px;padding:0;border-bottom:1px solid rgba(0,0,0,.05);border-right:1px solid rgba(0,0,0,.04);width:${w}px;min-width:${w}px;max-width:${w}px;overflow:hidden">
      ${leftStrip}
      <div class="g-cell-inner" style="display:flex;align-items:center;padding:0 10px 0 ${ipl}px;height:100%;gap:4px;overflow:hidden;min-width:0;cursor:text">
        ${dragHandle}${cellDisplay(colId,task,ctx)}
      </div>
    </td>`;
  }).join('');
  return `<tr class="g-row" data-id="${task.id}" style="background:${bg}">${cells}<td style="border-bottom:1px solid rgba(0,0,0,.05)"></td></tr>`;
}
...
export function renderGrid({ mount, tasks, people, deliverables, sprints, db, projectId, onSelect, onRerender }) {
  ...
  // Check if we need to focus a newly created task
  if (G.pendingFocusId) {
    const pfId = G.pendingFocusId;
    G.pendingFocusId = null;
    requestAnimationFrame(() => {
      const td = mount.querySelector(`td[data-id="${pfId}"][data-col="name"]`);
      if (td) activate(td);
    });
  }
  ...
  function deactivate() {
    ...
    const td = activeCell; activeCell = null;
    td.classList.remove('g-editing');
    const task = db.get('tasks', td.dataset.id);
    if (task) td.querySelector('.g-cell-inner').innerHTML = cellDisplay(td.dataset.col, task, ctx);
  }
  ...
  function activate(td) {
    ...
    const indentPx=colId==='name'?parseInt(inner.style.paddingLeft||'14'):10;
    ...
    const onSave=value=>{
      ...
      if(inner&&activeCell!==td) inner.innerHTML=cellDisplay(colId,updated,ctx);
      td.classList.remove('g-editing');
    };
    ...
    const onEnter=()=>{
      ...
      deactivate();
      // Create new row below and focus it
    ...
    const onTabDir=dir=>{
      ...
      if(next){const nextTd=mount.querySelector(`td[data-id="${taskId}"][data-col="${next}"]`);if(nextTd)requestAnimationFrame(()=>activate(nextTd));}
    };

    inner.innerHTML='';
    if(colId==='name'){
      const h=document.createElement('span');
      h.className='row-drag';h.style.cssText='opacity:0;cursor:grab;color:#D1D5DB;margin-right:5px;font-size:12px;flex-shrink:0;user-select:none';h.textContent='⠿';
      inner.appendChild(h);
    }
  ...
  table.addEventListener('mouseover',e=>{const r=e.target.closest('.g-row');if(r)r.querySelectorAll('.row-drag').forEach(h=>h.style.opacity='1');});
  table.addEventListener('mouseout', e=>{const r=e.target.closest('.g-row');if(r&&!r.matches(':hover'))r.querySelectorAll('.row-drag').forEach(h=>h.style.opacity='0');});
  ...
  table.addEventListener('click',e=>{
    ...
    G.pendingFocusId=newTask.id;  // focus after rerender using current activate
    onRerender();
  });
  ...
  table.addEventListener('contextmenu',e=>{
    ...
    const items=[
      ...
      {label:`<i class="ti ti-plus"></i> Insert row above`, action:()=>{
        const t=db.insert('tasks',{project_id:projectId,type:'task',name:'',status:'todo',parent_id:task.parent_id||null,deliverable_id:task.deliverable_id||null,effort_min:0,priority:'normal',progress:0,flagged:false,sort_order:(task.sort_order||1)-0.5});
        G.pendingFocusId=t.id; onRerender(); hideCtx();
      }},
      {label:`<i class="ti ti-plus"></i> Insert row below`, action:()=>{
        const t=db.insert('tasks',{project_id:projectId,type:'task',name:'',status:'todo',parent_id:task.parent_id||null,deliverable_id:task.deliverable_id||null,effort_min:0,priority:'normal',progress:0,flagged:false,sort_order:(task.sort_order||0)+0.5});
        G.pendingFocusId=t.id; onRerender(); hideCtx();
      }},
      null,
  ...
  mount.querySelectorAll('.g-col-hd').forEach(th=>{
    ...
    th.addEventListener('drop',()=>{
      ...
      if(G.dragCol&&G.dragOverCol&&G.dragCol!==G.dragOverCol&&!ALL_COLS.find(c=>c.id===G.dragCol)?.fixed){
  ...
  mount.querySelectorAll('.col-resize').forEach(handle=>{
    handle.addEventListener('mousedown',e=>{
      ...
      const colId=handle.dataset.col,startX=e.clientX,startW=G.widths[colId];
